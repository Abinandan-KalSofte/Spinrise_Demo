import { Card, Col, DatePicker, Form, Input, Row, Select, Space, Tag, Typography, Spin, Alert } from 'antd'
...
export function PRHeaderForm({ form }: Props) {
  // TODO: load dynamically when division selection is implemented
  ...
  return (
    ...
    >
      ...
      >        {departmentsError && (
          <Alert
            message="Department Loading Error"
            description="Failed to load departments from the server. Please try refreshing the page or contact support if the issue persists."
            type="error"
            showIcon
            style={{ marginBottom: 16 }}
            action={
              <Space>
                <a onClick={refetchDepartments} style={{ color: '#1890ff' }}>
                  Retry
                </a>
              </Space>
            }
        ...
        <Row gutter={[32, 0]}>
          ...
          <Col xs={24} md={12} xl={8}>
                ...
                notFoundContent={
                  departmentsLoading ? (
                    <div style={{ textAlign: 'center', padding: '8px' }}>
                      <Spin size="small" />
                      <div style={{ marginTop: 8 }}>Loading departments...</div>
                    </div>
                  ) : departmentsError ? (
            ...
            <Form.Item label="Section" name="section" rules={[{ max: 20 }]}>
              <Input placeholder="Enter section" maxLength={20} />
            </Form.Item>
          </Col>
