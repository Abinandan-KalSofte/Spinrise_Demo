...
public class AuthService : IAuthService
{
    ...
    {
        var user = await _authUserStore.ValidateCredentialsAsync(request.Email, request.Password);
        if (user is null)
        {
            return null;
        }
    ...
    public async Task<AuthResponseDto?> RefreshToken(RefreshTokenRequestDto request)
    {
        ...
        var user = await _authUserStore.GetByEmailAsync(refreshToken.User.Email);
        if (user is null)
        {
            return null;
        }
