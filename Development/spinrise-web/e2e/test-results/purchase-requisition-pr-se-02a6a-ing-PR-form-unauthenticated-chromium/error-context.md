...
- Name: purchase-requisition\pr-security.spec.ts >> PR Security — Authentication Guard >> should redirect to login when accessing PR form unauthenticated
- Location: e2e\tests\purchase-requisition\pr-security.spec.ts:184:3
...
Received string:  "http://172.16.16.40:3000/purchase/requisition/v1/new"
Timeout: 10000ms

Call log:
  - Expect "toHaveURL" with timeout 10000ms
    23 × unexpected value "http://172.16.16.40:3000/purchase/requisition/v1/new"
...
- banner:
  - img "Kalsofte"
  - text: SpinRise ERP Platform 14 May 2026 | 03:42:53 PM
- main:
...
```ts
  86  |    */
  87  |   test('should not execute any XSS payload across all text inputs', async ({ page }) => {
  88  |     const prFormPage = new PRFormPage(page)
  89  |     await prFormPage.goto()
  90  | 
  91  |     const alerts: string[] = []
  92  |     page.on('dialog', (dialog) => {
  93  |       alerts.push(dialog.message())
  94  |       void dialog.dismiss()
  95  |     })
  96  | 
  97  |     // Inject XSS payloads into all visible text inputs
  98  |     const inputs = await page.locator('input[type="text"], textarea').all()
  99  |     for (const input of inputs) {
  100 |       try {
  101 |         await input.fill(XSS_PAYLOADS[0])
  102 |         await page.keyboard.press('Tab')
  103 |       } catch {
  104 |         // Some inputs may be disabled — skip them
  105 |       }
  106 |     }
  107 | 
