...
export function PRHeaderCards({
  ...
  // Requisition Type values are fixed per FSD §3 (E/O/U) — not from lookup
      ...
      >
        ...
        <Row gutter={[16, 0]}>
          ...
          <Col xs={24} sm={8} md={8}>
            <Form.Item
              name="iType"
              label={<FL text="Requisition Type" />}
              rules={[{ required: true, message: 'Required' }]}
              style={ITEM}
            >
              <Select
        ...
        {/* Row 3: Created By (edit mode only) */}
        {createdBy && (
          <Row gutter={[16, 0]}>
            <Col xs={24} sm={8} md={8}>
              <Form.Item label={<FL text="Created By" />} style={ITEM}>
                <Input value={createdBy.trim()} disabled />
              </Form.Item>
            </Col>
          </Row>
        )}
