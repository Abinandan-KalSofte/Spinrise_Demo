...
'     sQry = "SHAPE {SELECT TOP 1 DIVCODE,ISSTYPE,DOCNO,DOCDT,slcode,OILCODE,OILWT,OILCODE1,OILWT1,WTRWT,cntcd,TINT,MODUSRID,ISSTO,ordno,vehicleno,MIXGRP,RDIVCODE,Driver,InvoiceNo,InvDate,TimeOut,Form,Formno,SoftWaste,ISSUEFLG FROM RM_ISSREQH  " & _
    ...
    sQry = "SELECT TOP 1 DIVCODE,ISSTYPE,DOCNO,DOCDT,slcode,OILCODE,OILWT,OILCODE1,OILWT1,WTRWT,cntcd,TINT,MODUSRID,ISSTO,ordno,vehicleno,MIXGRP,RDIVCODE,Driver,InvoiceNo,InvDate,TimeOut,Form,Formno,SoftWaste,ISSUEFLG FROM RM_ISSREQH  " & _
                        "  where DIVCODE = '" & Divcode & "' and  DOCDT BETWEEN '" & Format(yfdate, "yyyy-mm-dd") & "' AND '" & Format(yldate, "yyyy-mm-dd") & "' " & _
...
'         SSTab2.TabEnabled(1) = False
         ...
         Set adoSecondaryRS = New Recordset
         adoSecondaryRS.Open "SELECT DISTINCT a.DIVCODE,ISSTYPE,DOCNO,DOCDT,a.LOTNO""Lot No"",a.LOTDT""Lot Date"",b.LotType""Lot Type"",b.Plotno""Supplier Lot No"",VARname""Variety Name"",a.CATCD""Category"",a.baleno""Quantity""  FROM RM_ISSREQB a,rm_lot b,rm_var c  " & _
                        " where c.varcode=a.varcode and b.varcode=c.varcode and a.lotno=b.lotno and a.lotdt=b.lotdt and a.catcd=b.catcd and a.divcode=b.divcode and DOCDT BETWEEN '" & Format(yfdate, "yyyy-mm-dd") & "' AND '" & Format(yldate, "yyyy-mm-dd") & "' and  b.rejflg='N' and b.lotyear='" & Year(yfdate) & "' and a.docno=" & adoPrimaryRS("Docno") & " and a.docdt='" & Format(adoPrimaryRS("docdt"), "yyyy-MM-dd") & "'", DB, adOpenStatic, adLockBatchOptimistic
...
'   adoPrimaryRS.Open "SHAPE {SELECT DIVCODE,ISSTYPE,DOCNO,DOCDT,slcode,OILCODE,OILWT,OILCODE1,OILWT1,WTRWT,cntcd,TINT,MODUSRID,ISSTO,ordno,vehicleno,MIXGRP,RDIVCODE,Driver,InvoiceNo,InvDate,TimeOut,Form,Formno,SoftWaste FROM RM_ISSREQH  " & _
    ...
    adoPrimaryRS.Open "SELECT DIVCODE,ISSTYPE,DOCNO,DOCDT,slcode,OILCODE,OILWT,OILCODE1,OILWT1,WTRWT,cntcd,TINT,MODUSRID,ISSTO,ordno,vehicleno,MIXGRP,RDIVCODE,Driver,InvoiceNo,InvDate,TimeOut,Form,Formno,SoftWaste,ISSUEFLG FROM RM_ISSREQH  " & _
                     "  where DIVCODE = '" & Divcode & "' and docno = '" & Trim(txtfields(0).Text) & "' and DOCDT = '" & Format(MaskEdBox1(0).Text, "yyyy-mm-dd") & "' " & _
                     " Order by A.DOCNO", DB, adOpenStatic, adLockBatchOptimistic
   If Not adoPrimaryRS.EOF Then
