...
Begin VB.Form Frm_Rpt_StockStatment 
   ...
   LinkTopic       =   "Form2"
   MDIChild        =   -1  'True
   ScaleHeight     =   34972.91
   ScaleMode       =   0  'User
   ScaleWidth      =   5.21538e8
   ...
   End
End
Attribute VB_Name = "Frm_Rpt_StockStatment"
...
GLCAT:
    If CL_Cat = "" Then
        MsgBox "Please Select Any Category ", vbInformation, head
        Exit Sub
    End If
...
GREC:
    If CLREC = "" Then
        MsgBox "Please Select Any Receipt Type ", vbInformation, head
        Exit Sub
    End If
...
GLVAR:
    If CLVAR = "" Then
        MsgBox "Please Select Any Variety  ", vbInformation, head
        Exit Sub
    End If
...
GLSUP:
    If CLSup = "" Then
        MsgBox "Please Select Any Supplier  ", vbInformation, head
        Exit Sub
    End If
...
GLLOt:
    If CLLot = "" Then
        MsgBox "Please Select Any Lot  ", vbInformation, head
        Exit Sub
    End If
CGOD = ""
For I = 1 To List_Godown.ListItems.Count
        If List_Godown.ListItems.Item(I).Checked = True Then
         CGOD = "Y"
         GoTo GGOD
        End If
    Next
GGOD:
    If CGOD = "" Then
        MsgBox "Please Select Any Godown  ", vbInformation, head
        Exit Sub
    End If
...
L_aread_God = ""
          ...
          '  DB.BeginTrans
            ...
            For I = 1 To List_Lot.ListItems.Count
                If List_Lot.ListItems.Item(I).Checked Then
                     sRecSupp = Trim(List_Lot.ListItems(I).SubItems(1))
                     sql = "insert into RM_ProductprintDet(divcode,LotNo ,Lotdt ,Lotyear ,LotType ,catcd ,varcode ,Godcode,RecType ,supcd,IPadd) " & _
                    "  values( '" & Divcode & "','" & val(Trim(List_Lot.ListItems(I).SubItems(7))) & "' ,'" & Format(Trim(List_Lot.ListItems(I).SubItems(1)), "yyyy-mm-dd") & "','" & val(Trim(List_Lot.ListItems(I).SubItems(8))) & "' ,'" & Trim(Trim(List_Lot.ListItems(I).SubItems(2))) & "' ,'" & Trim(Trim(List_Lot.ListItems(I).SubItems(3))) & "' ,'" & Trim(Trim(List_Lot.ListItems(I).SubItems(4))) & "' " & _
                    " ,'" & L_aread_God & "' ,'" & Trim(Trim(List_Lot.ListItems(I).SubItems(6))) & "','" & Trim(Trim(List_Lot.ListItems(I).SubItems(9))) & "', '" & LocalIPAdd & "')"
                     DB.Execute sql
                End If
            Next I
           ' DB.CommitTrans
 ...
 Rs.Open "select div_printname,divname from pp_divmas where divcode='" & Divcode & "'", DB, adOpenStatic
 
 If Not Rs.EOF Then
    If UCase(CustID) = "PALLAVA" Or UCase(CustID) = "SHRIGIRI" Then
       divname = Rs("divname")
    Else
       divname = Rs("div_printname")
    End If
    
 End If
If OptRec(0).value Then
 Call StockLotReport2(CStr(Format(DTPicker1.value, "yyyy-mm-dd")), CStr(Format(DTPicker2.value, "yyyy-mm-dd")), "", "1", "1", "", "")
ElseIf OptRec(1).value Or OptRec(6).value Then
 
 Call StockVarietyReport2(CStr(Format(DTPicker1.value, "yyyy-mm-dd")), CStr(Format(DTPicker2.value, "yyyy-mm-dd")), "", "1")
 
ElseIf OptRec(3).value Then
 Set Rs = New Recordset
 Rs.Open "select div_printname,divname from pp_divmas where divcode='" & Divcode & "'", DB, adOpenStatic
 ...
 End If
  ...
  Call StockGodownReportCrystal(CStr(Format(DTPicker1.value, "yyyy-mm-dd")), CStr(Format(DTPicker2.value, "yyyy-mm-dd")), "", divname, CStr(X), CStr(Y), "N", "", "", "")
ElseIf OptRec(2).value Then
Call StockReportStationwise(CStr(Format(DTPicker1.value, "yyyy-mm-dd")), CStr(Format(DTPicker2.value, "yyyy-mm-dd")))
ElseIf OptRec(4).value Then
 
 Call StockVarietyLedger(CStr(Format(DTPicker1.value, "yyyy-mm-dd")), CStr(Format(DTPicker2.value, "yyyy-mm-dd")), "", "1")
...
Exit Sub
   ...
   ' MaskEdBox1.Text = DTPicker1.value
            ...
            Set clsCryRpt.cryRept = Cry_AllocationList
            ...
            With CrystalReport1
                 ...
                 .ReportFileName = KALFOLDERDATA & "RepPr.rpt"
                 .ParameterFields(0) = "@divcode;" & Divcode
                 .ParameterFields(1) = "@AtDate;" & Format(DTPicker1.value, "yyyy-MM-dd")
             
                 .ParameterFields(2) = "@depcode;" & L_Dep
                 .ParameterFields(3) = "@wcat;" & L_Cat
              
                 .ParameterFields(4) = "@subdep;A"
                 .ParameterFields(5) = "@IPadd;" & Trim(LocalIPAdd)
                 
          
                 
'                 .Formulas(0) = "@Heading1='Prepared By'"
'                 .Formulas(1) = "@Heading2='PM'"
'                 .Formulas(2) = "@Heading3='VP & MD'"
...
On Error GoTo DataCombo1_Change_Error
    ...
    If Repindex = 1 Then
       temprs.Open " Select Distinct 'A' Type, 'OWN' as description   UNION Select Distinct Issue_Code  type, Issue_Desc as description from rm_issuetype a INNER JOIN RM_Lot b ON a.Issue_Code=b.Transfertype and b.lotyear='" & Year(yfdate) & "' Order by Type", DB, adOpenStatic, adLockBatchOptimistic
    End If
  
    List_RecType.ListItems.Clear
    If temprs.RecordCount > 0 Then
        Do While Not temprs.EOF
            Set l1 = List_RecType.ListItems.ADD(, , temprs!Description)
            List_RecType.ListItems(List_RecType.ListItems.Count).ListSubItems.ADD , , temprs!Type
            temprs.MoveNext
        Loop
    End If
    temprs.Close
...
Private Sub OptRec_Click(Index As Integer)
    If OptRec(2).value = True Then
        Call load_StockDetailsGod
        Label4.Caption = "Station"
    ElseIf OptRec(2).value = False Then
        Call load_StockDetailsGod
         Label4.Caption = "Godown"
    End If
End Sub
...
End If
        ...
        If UCase(CustID) = "SHRIGIRI" Or UCase(CustID) = "PALLAVA" Or UCase(CustID) = "VSM" Then
            Set clsCryRpt1.cryRept = Cry_LotwiseStockStatement
        ElseIf UCase(CustID) = "SARANYA" Then
            Set clsCryRpt1.cryRept = Cry_LotwiseStkStmt_Ashok
        Else
  ...
  If Year(yfdate) = Year(F_Date) And Year(F_Date) = Year(T_Date) Then
   ...
   End If
    ...
    Set RSV = New Recordset
        ...
        strSQL = strSQL + "else (sum(isnull(a.recvalue,0))-(sum(isnull(a.PRODVALUE,0))+ sum(isnull(a.TRANSVALUE,0))+sum(isnull(a.SALESVALUE,0))+sum(isnull(a.JWVALUE,0))+sum(isnull(a.REJVALUE,0))-sum(isnull(ISSRETVAL,0)))) END CLOSEVALUE,SUM(ISNULL(A.SALESVALUE,0)) SALESVALUE,sum(isnull(a.SALESBAL,0)) SALESBAL,sum(isnull(a.SALESKGS,0)) SALESKGS "
        strSQL = strSQL + " FROM ("
...
Do While Not RSV.EOF
         ...
         VTOT = RSV("opbales") + val(RSV("opboras") & "")
         vtot1 = RSV("opboras")
         vtot2 = RSV("opkgs")
         vtot3 = RSV("recbales") + val(RSV("recboras") & "")
         VTOT4 = RSV("recboras")
         VTOT5 = RSV("reckgs")
        If UCase(CustID1) = "SCM" Then
            VTOT6 = (RSV("issbales") + val(RSV("issboras") & "")) - val(RSV("SALESBAL") & "")
            VTOT7 = RSV("issboras")
            VTOT8 = RSV("isskgs") - val(RSV("SALESKGS") & "")
            VTOT14 = RSV("issvalue") - val(RSV("SALESVALUE") & "")
            
            SALESBAL = val(RSV("SALESBAL") & "")
            SALESVALUE = val(RSV("SALESVALUE") & "")
            SALESKGS = val(RSV("SALESKGS") & "")
        Else
            VTOT6 = RSV("issbales") + val(RSV("issboras") & "")
            VTOT7 = RSV("issboras")
            VTOT8 = RSV("isskgs")
            VTOT14 = RSV("issvalue")
        ...
        VTOT9 = RSV("clobales") + val(RSV("cloboras") & "")
        VTOT10 = RSV("cloboras")
        VTOT11 = RSV("clokgs")
...
'   strSQL = "insert into Temp_per_stockstatementvar Values ('" & VarName & " ','" & (VTOT + vtot1) & "','" & vtot2 & "','" & VTOT12 & "','" & (vtot3 + VTOT4) & "','" & VTOT5 & "','" & VTOT13 & "','" & (VTOT6 + VTOT7) & "'," & VTOT8 & "," & VTOT14 & "," & (VTOT9 + VTOT10) & "," & VTOT11 & "," & VTOT15 & ")"
   If UCase(CustID1) = "SCM" Then
    DB.Execute "insert into Temp_per_stockstatementvar Values ('" & VarName & " '," & VTOT & "," & vtot1 & "," & vtot2 & "," & VTOT12 & "," & vtot3 & "," & VTOT4 & "," & VTOT5 & "," & VTOT13 & "," & VTOT6 & "," & VTOT7 & "," & VTOT8 & "," & VTOT14 & "," & vtotRejBales & "," & vtotRejKgs & "," & vtotRejval & "," & VTOT9 & "," & VTOT10 & "," & VTOT11 & "," & VTOT15 & ",'" & val(SALESVALUE) & "','" & val(SALESBAL) & "','" & val(SALESKGS) & "')"
   Else
...
'FrmRpt.Show
        ...
        Dim clsCryRpt1 As New clsCrystal
        If UCase(CustID1) = "SCM" Then
            Set clsCryRpt1.cryRept = Cry_StockStatment_VarietySCM  ' Rep_stock_varwise 'rep_stock_varwise1 '
        Else
            Set clsCryRpt1.cryRept = Cry_StockStatment_Variety
        End If
        clsCryRpt1.CrystalPrint
        Crr.Reset
        Crr.Connect = connectstring
        ...
        Crr.Formulas(8) = "UnitName='" & PrUnitName & "'"
        Crr.Formulas(9) = "Custid='" & CustID & "'"
...
Screen.MousePointer = 0
End Sub
Private Sub StockTransfertoAccess()
    Dim MSAcDBCon As ADODB.Connection
    
    Set MSAcDBCon = New Connection
    MSAcDBCon.CursorLocation = adUseClient
    MSAcDBCon.Provider = "MSDASQL.1"
    
    Dim RsMsAcces As New ADODB.Recordset
'    Set adoClone = SQLdata.Clone(adLockReadOnly)
    Set RsMsAcces = New ADODB.Recordset
    
        Set rsCon = New Recordset
        rsCon.Open "select  isnull(StkTransferAccess,'N')MsAccs_Tran_Flg, accesspath from rm_param", DB, adOpenStatic, adLockBatchOptimistic
        If Not rsCon.EOF Then
            If rsCon("MsAccs_Tran_Flg") = "Y" Then
                MSAcDBCon.Open "Driver={Microsoft Access Driver (*.mdb)};Dbq= " & rsCon("accesspath") & "\StockTransfer.mdb;"
            Else
                MsgBox "Please update the MS Access Data Transfer Flag from Paramater coming under General 3 tab", vbInformation, head
            End If
        End If

  
    Set SQLPHd = New Recordset
    SQLPHd.Open "select c.catcd,c.CATNAME,b.varcode,b.varname,a.clokgs from Temp_per_stockstatementvar a inner join rm_var b on a.varname=b.varname inner join rm_cat c on b.catcd=c.catcd where clokgs>0 order by b.varname", DB, adOpenStatic, adLockOptimistic
    
    RsMsAcces.Open "delete from StockTransfer", MSAcDBCon, adOpenStatic, adLockOptimistic, adLockReadOnly
    RsMsAcces.Open "SELECT * FROM StockTransfer where 1=2", MSAcDBCon, adOpenStatic, adLockOptimistic, adLockReadOnly
    I = 0
    
    SQLPHd.MoveFirst
 
        Do While Not SQLPHd.EOF
            I = I + 1
            
            RsMsAcces.AddNew
            RsMsAcces("catcode") = SQLPHd("catcd")
            RsMsAcces("catname") = SQLPHd("catname")
            RsMsAcces("varcode") = SQLPHd("varcode")
            RsMsAcces("varname") = SQLPHd("varname")
            RsMsAcces("RMClosingKgs") = SQLPHd("clokgs")

            RsMsAcces.UpdateBatch adAffectAllChapters
        ...
        RsMsAcces.Close
        MsgBox "Stock transferred to MS Access DB successfully", vbInformation, head
End Sub
